...
// --- AFFICHAGE CHIFFRE (0 ou 1) ---
// Style : Blanc, sans fond, posé sur le fil
void SchematicWidget::drawLogicLabel(QPainter &painter, int x, int y, int value)
{
    painter.save();
    ...
    // Zone de texte située juste au-dessus du fil (y - 18)
...
QPair<QPoint, int> SchematicWidget::drawRecursive(QPainter &painter, QString expr, int x, int y, int heightStep)
{
    ...
    if (splitIndex == -1) {
        ...
        // Police plus grande (taille 14)
        painter.setFont(QFont("Segoe UI", 14, QFont::Bold));

        // Texte décalé vers la gauche (x - 65) pour ne pas toucher le chiffre
...
int SchematicWidget::countLeaves(QString expr)
{
    expr = expr.trimmed();
    QString op;
    int splitIndex = findMainOperator(expr, op);
